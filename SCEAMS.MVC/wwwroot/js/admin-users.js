(() => {
  ...
  const confirmButton = dialog?.querySelector("[data-status-confirm]");
  const confirmLabel = dialog?.querySelector("[data-status-confirm-label]");
  const cancelButton = dialog?.querySelector("[data-status-cancel]");
  ...
  document.querySelectorAll("[data-status-action]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      const fullName = button.getAttribute("data-user-name") || "Tài khoản";
      ...
      if (typeof dialog.showModal === "function") {
        dialog.showModal();
      } else if (window.confirm(description?.textContent || "Xác nhận thao tác?")) {
        form.requestSubmit();
      }
    });
  });
