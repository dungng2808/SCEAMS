...
public sealed class ClubMembershipService : IClubMembershipService
{
    ...
    public async Task<Result<PagedResult<ClubMembershipResponseDto>>> GetActiveMembershipsAsync(
        int clubId,
        string? search,
        int page,
        int pageSize,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            return Result<PagedResult<ClubMembershipResponseDto>>.Fail(
                "Bạn không có quyền xem danh sách thành viên của câu lạc bộ này.",
                StatusCodes.Status403Forbidden);
        }
        ...
        var queryable = _unitOfWork.Clubs.GetQueryable()
            .Where(c => c.Id == clubId)
            .SelectMany(c => c.Memberships)
            .Where(m => m.Status == ClubMembershipStatus.Active);
        ...
        return Result<PagedResult<ClubMembershipResponseDto>>.Ok(
            new PagedResult<ClubMembershipResponseDto>(items, totalItems));
    }
